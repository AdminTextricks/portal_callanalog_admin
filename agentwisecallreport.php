	...
	<div class="section__content section__content--p30 page_mid">
		<div class="container-fluid">
			...
			<div class="overview-wrap table_top_heading">
				<a
					href="http://edu.bigpbx.com/bigpbxnew/bigpbx/acallrecord.php?start=<?php echo $_GET['start']; ?>&end=<?php echo $_GET['end']; ?>&username=<?php echo $_GET['username']; ?>">
					...
					<div class="table-responsive table-responsive-data2 big_live_dtl manage_queue_table_outer">
						...
						<table class="table manage_queue_table">
							...
							<tbody>
								...
								<?php
								$callingexten = "select name from cc_sip_buddies where accountcode = '" . $_GET['username'] . "'";
								$result_extens = mysqli_query($connection, $callingexten);
								$i = 1;
								while ($rowextens = mysqli_fetch_array($result_extens)) {

									$query_totalall = "SELECT cc.starttime AS starttime, cc.stoptime AS stoptime, cc.sessiontime AS sessiontime, cc.calledstation AS calledstation, cc.sessionbill AS sessionbill FROM `cc_call` cc LEFT JOIN cc_card cd ON cc.card_id = cd.id WHERE cc.starttime BETWEEN '" . $start . "' AND '" . $end . "' AND cd.username ='" . $_GET['username'] . "' and terminatecauseid=1 and cc.sessiontime !='0' and cc.sessionbill !='0' and cc.calledstation ";
									// $query_totalall = "SELECT cc.uniqueid as uniqueid, count(cc.id) as totalcall , sum(cc.duration) as duration, sum(cc.billsec) as sessionbills FROM `cdr` cc LEFT JOIN cc_sip_buddies cd ON cc.extension = cd.name WHERE cc.calldate BETWEEN '".$start."' AND '".$end."' AND cc.disposition='ANSWERED' and cc.duration !='0' and cc.billsec !='0' and cd.name = '".$rowextens['name']."'";
...
// $result_waiting = mysqli_query($connection , $query_waiting_call);
										...
										// {
										$agentname = "SELECT sip.agent_name AS agent_name,sip.name as extension FROM cc_sip_buddies sip where sip.name = '" . $rowextens['name'] . "'";
										...
										<tr style="color:white;text-align: left;">
											...
											<td style="text-align: left;">
												<?php echo $rowcon['totalcall']; ?>
											</td>
											<td style="text-align: left;">
												<?php if (count($rowcon['duration']) > 0) {
													echo $rowcon['duration'];
												} else {
													echo '0';
												} ?>
											</td>
											...
											<td style="text-align: left;">
												<?php if (count($rowcon['sessionbills']) > 0) {
													echo $rowcon['sessionbills'];
												} else {
													echo '0';
												} ?>
											</td>
