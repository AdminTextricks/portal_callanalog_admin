...
$query_cdr_pagination = "SELECT count(id) FROM `cdr` where calldate between '".$fromdates."' and '".$todates."' ".$disposition." ".$CALLTYPE." ".$queueNames." ".$extensions." ".$DIDSS." ".$CLID." ".$DNID."";
...
$query_cdr = "SELECT * FROM `cdr` where calldate between '".$fromdates."' and '".$todates."' ".$disposition." ".$CALLTYPE." ".$queueNames." ".$extensions." ".$DIDSS." ".$CLID." ".$DNID." order by id DESC limit ".$offset.", ".$total_records_per_page."";
...
$urlcode = 'fromDate='.$_GET['fromDate'].'&hoursFrom='.$_GET['hoursFrom'].'&minutesFrom='.$_GET['minutesFrom'].'&toDate='.$_GET['toDate'].'&hoursTo='.$_GET['hoursTo'].'&minutesTo='.$_GET['minutesTo'].'&submit='.$_GET['submit'].'&showRecords='.$_GET['showRecords'].'&pageNumber='.$_GET['pageNumber'].'&extension='.$_GET['extension'].'&call_type='.$_GET['call_type'].'&queueName='.$_GET['queueName'].'&DID='.$_GET['DID'].'&CLID='.$_GET['CLID'].'&DNID='.$_GET['DNID'].'&disposition='.$_GET['disposition'].'';
...
<h2 class="title-1"> Reports </h2><?php //echo $query_cdr; ?>
</div>
</div>
</div>
...
<option <?php if($_GET['disposition'] == 'ANSWER') { echo 'selected="selected"'; } else { echo ''; } ?> value="ANSWER">ANSWERED</option>
...
<option <?php if($_GET['disposition'] == 'CHANUNAVAIL') { echo 'selected="selected"'; } else { echo ''; } ?> value="CHANUNAVAIL">CHANUNAVAIL</option>
<option <?php if($_GET['disposition'] == 'TIMEOUT') { echo 'selected="selected"'; } else { echo ''; } ?> value="TIMEOUT">TIMEOUT</option>
</select>
</div>
</div>
...
<input id="CLID" name="CLID" class="form-control" type="text" value="<?php echo $_GET['CLID']; ?>">
</div>
</div>

<!-- <div class="col-md-4">
<div class="form-group">
<label for="text-input" class=" form-control-label" style="color:black;margin-left:0px;font-weight:bold;">Dialed No.</label>
<input id="DNID" name="DNID" class="form-control" type="text" value="<?php echo $_GET['DNID']; ?>">
</div>
</div> -->
...
<td><?php echo $row_cdr['caller_num']; ?></td>
<td><?php echo $row_cdr['DID']; ?></td>
<td><?php echo $row_cdr['destination']; ?></td>
<td><?php echo $row_cdr['extension']; ?></td>
<td><?php if($row_cdr['call_type'] == '0') { echo 'Outbound'; } else { echo 'Inbound'; } ?></td>
<td><?php echo seconds2human($row_cdr['duration']); ?></td>
<td><?php echo $row_cdr['recvip']; ?></td>
<td><audio type="audio/wav" src="/recordings/<?php if($row_cdr['disposition'] == 'ANSWER') { echo $row_cdr['Recording']; } else { echo ''; } ?>" controls="" controlslist="nodownload" > </audio></td>
</tr>
<?php $i++; } } else { ?>
